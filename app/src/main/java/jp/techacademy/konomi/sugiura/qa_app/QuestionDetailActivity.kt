package jp.techacademy.konomi.sugiura.qa_app
...
class QuestionDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var mIsCreateFavorite = false
        favoriteRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val data = snapshot.value as Map<*, *>?
                if( snapshot.value == null ){
                    mIsCreateFavorite = false
                    favorite_button.setBackgroundResource(R.drawable.favorite_button_default)
                }else{
                    mIsCreateFavorite = true
                    favorite_button.setBackgroundResource(R.drawable.favorite_button_pressed)
                }
            }
        ...
        favorite_button.setOnClickListener{
            if(mIsCreateFavorite){
                ...
                favorite_button.setBackgroundResource(R.drawable.favorite_button_default)
                Snackbar.make(findViewById(android.R.id.content), "お気に入りから削除しました", Snackbar.LENGTH_LONG).show()
                mIsCreateFavorite = false
            }else{
                val data = HashMap<String, String>()
                data["favorite"] = mQuestion.questionUid
